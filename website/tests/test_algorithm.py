...
from website.models import Candidate, City
import json
...
class GetGradeTest(TestCase):
    ...
    def test_grade_is_the_lowest_for_selected_city_and_specialty(self):
        ...
        candidates = Candidate.objects.filter(
            choice__name=specialty,
            location__name=city).order_by('grade')
        response = self.client.get(reverse('get_grade'), {
            'city': city,
            'specialty': specialty})
        self.assertEqual(
            str(candidates.first().grade),
...
class GetSpecialtyTest(TestCase):
    ...
    def test_using_fake_city_return_empty_list(self):
        response = self.client.get(
            reverse('get_specialty'),
            {'city': 'qdzqdsdqzdq', 'grade': '10'})
...
class GetCityTest(TestCase):
    ...
    def test_using_fake_specialty_return_empty_list(self):
        response = self.client.get(
            reverse('get_city'),
            {'specialty': 'qdzqdsdqzdq', 'grade': '10'})
    ...
    def test_using_fake_grade_return_empty_error(self):
        with self.assertRaises(ValueError):
            self.client.get(
                reverse('get_specialty'),
                {'specialty': 'Anestezie şi Terapie Intensivă', 'grade': 'ten'})
