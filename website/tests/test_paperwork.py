...
from website.models import Paperwork_Service, Paperwork
...
class GetListofPaperWorkTest(TestCase):
    ...
    def test_return_right_list_of_paperwork(self):
        ...
        pw_service = Paperwork_Service.objects.get(
            service=service,
            paperwork=paperwork)
        new_paperwork_service_user = Paperwork_Service_User(
            user=user,
            pw_service=pw_service)
        new_paperwork_service_user.save()
        paperwork_service_user = Paperwork_Service_User.objects.filter(
            user=user,
            pw_service__service=service)
        list_of_paperwork = []
...
class AddToPaperWorksTest(TestCase):
    ...
    def test_fake_service_return_error(self):
        ...
        with self.assertRaises(IntegrityError):
            self.client.get(
                reverse('add_to_paperworks'),
                {
                    'user': user,
                    'paperwork': paperwork_name,
    ...
    def test_paperwork_is_added_with_success(self):
        ...
        pw_service = Paperwork_Service.objects.filter(
            service=service_id,
            paperwork__name=paperwork_name).first()
        self.client.get(
            reverse('add_to_paperworks'),
            {
                'user': user,
                'paperwork': paperwork_name,
                'service_id': service_id})
        documents = Paperwork_Service_User.objects.filter(
            user=user,
            pw_service=pw_service)
        paperwork_list = []
...
class RemoveToPaperWorkTest(TestCase):
    ...
    def test_fake_service_return_error(self):
        ...
        service_id = 999
        paperwork_name = 'Identification Document'
        Paperwork_Service.objects.filter(
            service=service_id,
            paperwork__name=paperwork_name).first()
        with self.assertRaises(AttributeError):
            self.client.get(
                reverse('remove_to_paperworks'),
                {
                    'user': user,
                    'paperwork': paperwork_name,
                    'service_id': service_id
